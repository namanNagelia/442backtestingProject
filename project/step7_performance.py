"""Step 7: OOS performance analytics per the BUFN 442 spec.

Pipeline:
  1. Assign each OOS stock-year to a decile (1..10) by SCORE within RETURN_YEAR.
  2. Join the decile map to monthly CRSP returns for the OOS years so every
     stock gets its decile label for every month in that year.
  3. Equal-weight the stocks in each decile each month → monthly decile returns.
  4. Long-short spread = D10 (long) − D1 (short), monthly.
  5. Regress the monthly L/S series on (a) MKT_RF for CAPM α and (b)
     MKT_RF + SMB + HML + UMD for the 4-factor α.
  6. Raw return (annualized), CAPM α (annualized), 4-factor α (annualized),
     and Information Ratio = annualized 4F α / annualized residual σ.

Notes:
  - The L/S spread is self-financing, so we regress it directly without
    subtracting RF (the RF terms cancel on both sides).
  - IC and yearly decile returns are kept as supplementary diagnostics.
"""
...
def evaluate(
    ...
    print(
        f"OOS monthly rows: {len(joined):,} "
        f"({joined['YYYYMM'].min()}-{joined['YYYYMM'].max()})"
    )

    # ---------- 3. Decile and L/S monthly returns ----------
    monthly_dec = (
        joined.groupby(["YYYYMM", "decile"])["RET"]
        .mean()
        .unstack("decile")
        .sort_index()
    )
    ...
    monthly_port["ls"] = monthly_port["long"] - monthly_port["short"]
    monthly_port["cumulative_ls"] = (1.0 + monthly_port["ls"]).cumprod() - 1.0
    monthly_port = monthly_port.reset_index()

    # ---------- 4. Merge with FF factors and run regressions ----------
    ...
    # IR = annualized 4F alpha / annualized residual std
    #    = (alpha_m * 12) / (resid_std_m * sqrt(12))
    #    = (alpha_m / resid_std_m) * sqrt(12)
    ...
    def _coef_table(fit: sm.regression.linear_model.RegressionResultsWrapper) -> pd.DataFrame:
        return pd.DataFrame(
            {
    ...
    # ---------- 6. Information Coefficient (supplementary, annual) ----------
    ic = (
    ...
    print(
        f"Raw: {raw_mean_ann:.2%}/yr (σ {raw_std_ann:.2%}, Sharpe {sharpe_ann:.2f}), "
        f"cum {cum_total:.2%}"
    )
    print(
        f"CAPM α: {alpha_capm_m * 12:.2%}/yr (t={t_alpha_capm:.2f}), β_mkt={beta_capm:.2f}"
    )
    print(
        f"4F α: {alpha_4f_m * 12:.2%}/yr (t={t_alpha_4f:.2f}), IR={ir:.2f}, R²={ff4.rsquared:.2f}"
    )
    ...
    return {
        ...
        "reg_coefs_capm": reg_coefs_capm,
        "reg_coefs_4f": reg_coefs_4f,
        "summary": summary,
